...
public class CodeTerminatorProtocol {
	...
	public void ProcessRequest(CodeTerminatorSocketMessage msg){
		switch(msg.MessageType){
			...
			case Broadcast_Warrior_Died:
				...
				if(Player.HasTarget && ClientsTarget != null && ClientsTarget.Name.equals(msg.TargetWarrior.Name)){
			...
			case Attack_Outcome:
				if(Player.Name.equals(msg.TargetWarrior.Name)){
					...
					{
						if (!Player.Type.startsWith("Zombie"))
						{
							//if you're not currently a zombie...
							if(msg.TargetAttack.StaminaCost < 4)
							{
								//and the attack that just killed you wasn't a "strong" attack, you can turn into a zombie if you want
								SetClientNotificationMessage(String.format("You have been slain, but may rise again as a zombie for one last attempt at glory...\r\nDo you rise?  (yes/no)"));
								RenderPlayerScreen();
								
								//for knowing how to handle the user response, go to the ProcessClientAction Method
							}
							else{
								System.out.println("You have been slain! \r\nThe attack was too strong to allow your body to rise again as a zombie.\r\nEnter any action to close the game:");
								Disconnect = true;
							}
						}
						else 
						{
							System.out.println("You have been slain... Again! \r\nEnter any action to close the game:");
							Disconnect = true;
						}
					}
				}
	...
	public void ProcessClientAction(String rawInput){
			...
			default:
				if(Player.CurrentHealth == 0){
					switch(rawInput){
						...
						case "no":
						case "n":
							System.out.println("You seek the comfort of the grave, enjoy your eternal slumber...\r\nEnter any action to close the game:");
							Disconnect = true;
							break;
					}
				}
